...
static int process_open_cmd(client_t *client, netiso_open_cmd *cmd)
{
	...
	else
	{
		...
		else
		{
			...
			if (viso != VISO_NONE || BE64(st.file_size) > 0x400000UL)
				printf("open %s\n", filepath + root_len);
			...
			{
				char buffer[0x10] = "";
				client->CD_SECTOR_SIZE = 0;

				client->ro_file->seek(0x8020UL, SEEK_SET);
				client->ro_file->read(buffer, 0xC);
				if (memcmp(buffer, "PLAYSTATION ", 0xC) == 0)
					client->CD_SECTOR_SIZE = 2048;
				else
				{
					client->ro_file->seek(0x9220UL, SEEK_SET);
					client->ro_file->read(buffer, 0xC);
					if (memcmp(buffer, "PLAYSTATION ", 0xC) == 0)
						client->CD_SECTOR_SIZE = 2336;
					else
					{
						client->ro_file->seek(0x9320UL, SEEK_SET);
						client->ro_file->read(buffer, 0xC);
						if (memcmp(buffer, "PLAYSTATION ", 0xC) == 0)
							client->CD_SECTOR_SIZE = 2352;
						else
						{
							client->ro_file->seek(0x9920UL, SEEK_SET);
							client->ro_file->read(buffer, 0xC);
							if (memcmp(buffer, "PLAYSTATION ", 0xC) == 0)
								client->CD_SECTOR_SIZE = 2448;
						}
					}
				}

				if (client->CD_SECTOR_SIZE > 0)
...
static int process_read_dir_cmd(client_t *client, netiso_read_dir_entry_cmd *cmd)
{
	...
	if (root_len > 2 && dirpath_len > (root_len + 1) && strncmp(client->dirpath, root_directory, root_len) == 0)
	{
		...
		for (int drive = 'C'; drive <= 'Z'; drive++)
		{
			...
			if (stat_file(client->dirpath, &st) < 0)
				continue;
...
int main(int argc, char *argv[])
{
	...
	if (ignore_drives)
	{
		...
		for (uint8_t d = 0; d < ignore_drives_len; d++)
			if ((ignore_drives[d] >= 'a') && (ignore_drives[d] <= 'z'))
				ignore_drives[d] -= ('a' - 'A');
	}
#endif
